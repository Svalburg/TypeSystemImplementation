...
class IfTest: public ::testing::Test
{
	protected:
		...
		void SetUp()
		{
			testRule = new RuleIf(new RuleVar("x"), new RuleAssign("y", new RuleCallCmpF("test", "0", new RuleConst("0"))), 
			new RuleAssign("y", new RuleCallCmpF("test", "1", new RuleConst("0"))));
			startState = new StateTuple();
			startState->declarePState("x", 0);
			vector<ComponentFunction*> compfuncs;
			vector<Function*> funcs;
			vector<TimeDependentEC*> tdec;
			ComponentFunction* cfunc0 = new ComponentFunction("test", "0", "z", new RuleConst("0"), new RuleConst("0"), 5);
			ComponentFunction* cfunc1 = new ComponentFunction("test", "1", "z", new RuleConst("1"), new RuleConst("1"), 5);
...
TEST_F(IfTest, State)
{
	...
	EXPECT_EQ(1, endState.getPStateValue("y"));
	startState->declarePState("x", 1);
	endState = testRule->sigma(*startState);
	EXPECT_EQ(0, endState.getPStateValue("y"));
}
